...
export default function CompanyWisePage() {
  ...
  const toggleSolved = (id: string) => {
    setSolvedMap(prev => {
      ...
      localStorage.setItem("hirenza-company-solved", JSON.stringify(next));
      return next;
    });
  };
  ...
  const toggleBookmark = (id: string) => {
    setBookmarks(prev => {
      const next = { ...prev, [id]: !prev[id] };
      localStorage.setItem("hirenza-company-bookmarks", JSON.stringify(next));
      return next;
    });
  };
  ...
  // Readiness Score function for a company
  const calculateReadiness = (company: Company) => {
    const solvedCount = company.problems.filter(p => solvedMap[p.id]).length;
    if (company.problems.length === 0) return 0;
    return Math.round((solvedCount / company.problems.length) * 100);
  };

  const filteredCompanies = useMemo(() => {
    return companies.filter(c => c.name.toLowerCase().includes(search.toLowerCase()) || c.description.toLowerCase().includes(search.toLowerCase()));
  }, [search]);
  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <p className="text-secondary">Target high-frequency interview patterns with automated Company Readiness Scores and curated problem archives.</p>
      </div>
        ...
        <>
          <div className="mb-6">
            <div className="flex items-center gap-3 bg-surface-2 border border-border rounded-xl px-4 py-3">
              ...
              <input
                type="text"
                placeholder="Search companies (e.g. Google, Amazon, Microsoft, Meta, Apple...)"
                value={search}
            ...
            {filteredCompanies.map(company => {
              ...
              return (
                <div 
                  ...
                  className="card p-6 cursor-pointer hover:border-purple-500/40 transition-all flex flex-col justify-between"
                >
                  <div>
                    <div className="flex items-center justify-between gap-3 mb-4">
                      <div className="flex items-center gap-3">
                        <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-surface-3 to-surface-2 border border-border flex items-center justify-center font-extrabold text-xl text-primary shadow-sm">
                          {company.logo}
                        </div>
                        <div>
                      ...
                      <div className="text-right">
                        <span className={`text-xs font-bold px-2 py-0.5 rounded-full border ${
                          readiness >= 75 ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/20" :
                          readiness >= 30 ? "bg-amber-500/10 text-amber-400 border-amber-500/20" :
                  ...
                  <div className="flex items-center justify-between pt-3 border-t border-border text-xs">
                    ...
                    <span className="text-purple-1 font-bold">Start Practice →</span>
                  </div>
                </div>
              );
            })}
      ...
      {selectedCompany && (
        <div>
          <button 
            ...
            className="mb-6 inline-flex items-center gap-1.5 text-xs text-purple-1 hover:underline font-bold"
          >
          ...
          <div className="card p-6 mb-6 bg-gradient-to-br from-surface-1 to-surface-2 border-purple-500/20">
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
              <div className="flex items-center gap-4">
                <div className="w-16 h-16 rounded-2xl bg-surface-3 border border-border flex items-center justify-center font-extrabold text-3xl shadow-inner">
              ...
              <div className="flex items-center gap-4 bg-surface-3 p-4 rounded-2xl border border-border">
                <div className="text-center">
                  <div className="text-3xl font-extrabold text-purple-1">{calculateReadiness(selectedCompany)}%</div>
                  <div className="text-[10px] uppercase font-bold text-muted mt-0.5">Readiness Score</div>
                </div>
                <div className="w-px h-10 bg-border" />
                <div className="text-xs text-muted space-y-0.5">
                  <p>Target: <strong>80%+</strong> to pass OA</p>
                  <p>Solved: <strong>{selectedCompany.problems.filter(p => solvedMap[p.id]).length}</strong> / {selectedCompany.problems.length}</p>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors flex items-center gap-1.5 ${
                  highFreqOnly ? "bg-amber-500/20 text-amber-300 border border-amber-500/30" : "bg-surface-3 text-muted hover:text-secondary"
                }`}
              >
              ...
              return (
                <div 
                  ...
                  className={`border rounded-xl p-4 transition-all flex items-center justify-between gap-4 ${
                    isSolved ? "bg-green-950/15 border-green-500/30" : "bg-surface-2 border-border hover:border-purple-500/20"
                  }`}
                >
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 flex-wrap mb-1">
                        <h4 className={`font-semibold text-sm ${isSolved ? "text-emerald-300 line-through opacity-85" : "text-primary"}`}>
                          {problem.title}
                        </h4>
                  ...
                  {problem.leetcodeUrl && (
                    <a
                      ...
                      className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-surface-3 hover:bg-surface-hover text-purple-1 rounded-lg text-xs font-bold border border-border shrink-0 transition-colors"
                    >
