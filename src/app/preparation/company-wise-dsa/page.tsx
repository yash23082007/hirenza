...
function CompanyWiseContent() {
  ...
  const handleSelectCompany = (id: string | null) => {
    if (id) {
      router.replace(`/preparation/company-wise-dsa?company=${id}`, { scroll: false });
    } else {
      router.replace("/preparation/company-wise-dsa", { scroll: false });
    }
  };
...
export default function CompanyWisePage() {
  return (
    <Suspense fallback={<div className="p-8 text-center text-muted">Loading company preparation...</div>}>
      <CompanyWiseContent />
    </Suspense>
  );
}
