"use client";

import { systemDesignPlaylists } from "@/data";
import { ExternalLink, Network, ArrowLeft } from "lucide-react";
import Link from "next/link";
import { useParams } from "next/navigation";
import { useMemo } from "react";
import { ResourceCard } from "@/components/ui/primitives/ResourceCard";

export default function SystemDesignPlaylistDetailPage() {
  const params = useParams();
  const playlistId = params.playlistId as string;

  const filteredPlaylists = useMemo(() => {
    if (!playlistId) return systemDesignPlaylists;
    const match = systemDesignPlaylists.filter(p =>
      p.id.toLowerCase() === playlistId.toLowerCase() ||
      p.title.toLowerCase().includes(playlistId.toLowerCase()) ||
      (p.tag && p.tag.toLowerCase().includes(playlistId.toLowerCase()))
    );
    return match.length > 0 ? match : systemDesignPlaylists;
  }, [playlistId]);

  return (
    <div>
      <div className="mb-8">
        <Link
          href="/preparation/system-design-playlists"
          className="inline-flex items-center gap-1.5 text-sm text-muted hover:text-purple-1 transition-colors mb-4"
        >
          <ArrowLeft size={14} /> All System Design Tracks
        </Link>
        <h1 className="text-3xl md:text-4xl font-extrabold tracking-tight mb-2">System Design Learning Tracks</h1>
        <p className="text-secondary">Curated architectural deep dives, distributed systems patterns, and low-level design masterclasses.</p>
      </div>

      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredPlaylists.map(playlist => (
          <ResourceCard
            key={playlist.id}
            href={playlist.url}
            target="_blank"
            rel="noopener noreferrer"
            className="group !p-0 !gap-0 overflow-hidden"
            visual={
              <div className={`aspect-video bg-gradient-to-br ${playlist.gradient || "from-slate-800 to-indigo-950"} p-5 flex flex-col justify-between relative overflow-hidden shadow-inner`}>
                <div className="flex items-center justify-between z-10">
                  <span className="text-[10px] font-bold uppercase tracking-wider bg-black/50 backdrop-blur-md px-2.5 py-1 rounded-full text-white/90 border border-white/10">
                    {playlist.tag || "System Design"}
                  </span>
                  <span className="text-[11px] font-bold bg-black/50 backdrop-blur-md px-2 py-0.5 rounded-md text-white/90">
                    {playlist.videos} Modules
                  </span>
                </div>

                <div className="z-10">
                  <span className="text-xs text-white/80 font-medium block mb-0.5">{playlist.author}</span>
                  <h3 className="text-sm font-bold text-white line-clamp-2 leading-snug drop-shadow-sm">
                    {playlist.title}
                  </h3>
                </div>
              </div>
            }
          >
            <div className="p-4 pt-0">
              <p className="text-xs text-secondary line-clamp-2 leading-relaxed">
                {playlist.description}
              </p>
            </div>
            
            <div className="p-4 pt-0 mt-auto border-t border-border-soft flex flex-row items-center justify-between text-xs w-full pb-3 mb-1">
              <span className="text-muted flex items-center gap-1 font-medium mt-3">
                <Network size={14} className="text-purple-1" /> Architecture Guide
              </span>
              <span className="text-purple-1 font-bold group-hover:underline inline-flex items-center gap-1 mt-3">
                Explore Resource <ExternalLink size={11} />
              </span>
            </div>
          </ResourceCard>
        ))}
      </div>
    </div>
  );
}
