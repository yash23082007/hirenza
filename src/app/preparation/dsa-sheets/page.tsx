...
function DSASheetsContent() {
  const searchParams = useSearchParams();
  const sheetParam = searchParams.get("sheet");

  const selectedSheet = useMemo(() => {
    return sheetParam ? dsaSheets.find(s => s.id === sheetParam) : null;
  }, [sheetParam]);
  ...
  const updateStatus = (problemId: string, status: ProblemStatus) => {
    setProblemStatuses(prev => {
      ...
      localStorage.setItem("hirenza-dsa-status", JSON.stringify(next));
      return next;
    });
  };
  ...
  const toggleBookmark = (problemId: string) => {
    setBookmarks(prev => {
      ...
      localStorage.setItem("hirenza-dsa-bookmarks", JSON.stringify(next));
      return next;
    });
  };
  ...
  const solvedCount = useMemo(() => {
    return allSheetProblems.filter(p => problemStatuses[p.id] === "solved" || problemStatuses[p.id] === "mastered").length;
  }, [allSheetProblems, problemStatuses]);

  const masteredCount = useMemo(() => {
    return allSheetProblems.filter(p => problemStatuses[p.id] === "mastered").length;
  }, [allSheetProblems, problemStatuses]);

  const reviewCount = useMemo(() => {
    return allSheetProblems.filter(p => problemStatuses[p.id] === "review").length;
  }, [allSheetProblems, problemStatuses]);
  ...
  const nextProblem = useMemo(() => {
    return allSheetProblems.find(
      p => !problemStatuses[p.id] || problemStatuses[p.id] === "unsolved" || problemStatuses[p.id] === "attempted"
    );
  }, [allSheetProblems, problemStatuses]);
...
export default function DSASheetsPage() {
  return (
    <Suspense fallback={<div className="p-10 text-muted">Loading DSA Sheets...</div>}>
      <DSASheetsContent />
    </Suspense>
  );
}
