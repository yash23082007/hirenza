...
function MostAskedQuestionsContent() {
  ...
  const handleSelectTech = (techId: string | null) => {
    if (techId) {
      router.replace(`/preparation/most-asked-questions?tech=${techId}`, { scroll: false });
    } else {
      router.replace("/preparation/most-asked-questions", { scroll: false });
    }
  };
...
export default function MostAskedQuestionsPage() {
  return (
    <Suspense fallback={<div className="p-8 text-center text-muted">Loading tech questions...</div>}>
      <MostAskedQuestionsContent />
    </Suspense>
  );
}
